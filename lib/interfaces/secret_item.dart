...
// dart-dbus generate-remote-object ./interfaces/org.freedesktop.Secret.Item.xml

import 'dart:io';
import 'package:dbus/dbus.dart';

class SecretItem extends DBusRemoteObject {
  SecretItem(DBusClient client, String destination, DBusObjectPath path) : super(client, name: destination, path: path);
  ...
  /// Gets org.freedesktop.Secret.Item.Type
  Future<String> getType() async {
    var value = await getProperty('org.freedesktop.Secret.Item', 'Type', signature: DBusSignature('s'));
    return value.asString();
  }

  /// Sets org.freedesktop.Secret.Item.Type
  Future<void> setType(String value) async {
    await setProperty('org.freedesktop.Secret.Item', 'Type', DBusString(value));
  }
